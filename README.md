...
本工具启动的是已安装的官方 ChatGPT 桌面应用。点击“启动”时才检查安装；未安装时提示前往 Microsoft Store，并保留设置窗口，安装完成后可再次点击“启动”。

打开启动器始终显示连接设置，不自动启动客户端。只有激活底部“启动”按钮才会启动；没有启动倒计时、空格跳转或输入框 Enter 默认启动。启动请求成功后启动器退出；失败时显示错误并留在设置页，允许重试。关闭设置窗口不停止已运行的 ChatGPT。
...
2. 按需填写“HTTP 代理”，点击当前 Tab 的“保存配置”，再点击底部“启动”。
3. 在官方客户端完成该账号的登录。下次打开连接设置时，启动器自动识别并收录登录账号。
4. 重复以上步骤添加其他账号；之后从账号列表选择已保存的账号，点击当前 Tab 的“保存配置”，再点击底部“启动”即可切换。

账号支持重命名和删除。登录与令牌刷新由官方客户端负责，启动器保存切换前的最新凭据；在客户端退出登录后，不会恢复该账号的旧令牌。本地存在凭据不代表账号仍然有效，失效时需在客户端重新登录。

账号或 API 条目缺少名称、名称为空白时，统一显示“未命名配置”，仍可重命名。重命名只修改草稿中的名称，保留条目 ID、凭据与其他字段，点击“保存配置”后落盘。
...
3. 点击当前 Tab 的“保存配置”，再点击底部“启动”。需要另一套 API 时，继续添加，或复制已有配置后修改。
4. 从配置列表选择目标 API，点击当前 Tab 的“保存配置”，再点击底部“启动”即可切换。

每套 API 单独保存连接信息和模型设置，支持复制、重命名和删除。“API 配置名称”用于区分列表条目，“提供者显示名称”是写入客户端配置的显示名称。

![自定义 API 设置](docs/images/custom-mode.png)

### 保存与切换何时生效

切换 Tab、选择列表条目以及添加、编辑、重命名、删除操作只改变当前草稿。点击当前 Tab 内的“保存配置”写入配置并留在设置页，不启动或停止 ChatGPT；底部“启动”只读取已保存配置，不隐式保存。存在未保存的字段、列表或模式修改时，启动会被阻止并提示先保存。启动器与已运行的 ChatGPT 进程不互斥，启动请求成功后启动器自动退出，不等待或检测客户端窗口。

底部“关闭”、标题栏关闭和 Esc 使用相同规则：没有未保存修改时直接退出；有修改时询问是否放弃，默认继续编辑。关闭不会撤销已经保存的配置，也不会停止 ChatGPT。
...
修改代理后需点击当前 Tab 的“保存配置”，再点击底部“启动”，使新设置进入客户端进程。浏览器中的外部登录页面和 Microsoft Store 是独立应用，不受启动器代理设置控制；自行清理环境的工具、WSL 和远程主机也不保证继承代理。
...
需要将本地历史关联到自定义 API 时，在“自定义 API”Tab 点击“修复对话”。点击后先说明修改范围并确认，修复前自动备份，过程中显示扫描、统计、备份、修复各阶段的真实 `n/total` 进度；失败时尝试回滚，且不继续启动客户端。
...
这些文件包含私人配置，不进入源码仓库。`.env` 与凭据、活动配置、配置库一起保存；任一文件保存失败时，启动器尝试恢复原状态并阻止启动。代理区块标记缺失、顺序错误或重复时，在保存前提示冲突。

已有配置中的显式登录限制、非文件凭据存储、选中的 `profile` 或与目标模式冲突的路由覆盖，可能阻止切换。启动器会在写入配置前提示，保留原配置供核对。

保存和启动前比较设置打开时的四文件基线：`config.toml`、`auth.json`、`launcher-profiles/modes.json` 与 `.env`。外部变化会阻止操作并保留草稿，需要重新打开设置；当前保守地将 OAuth 正常刷新也视为变化。启动只读取活动配置与认证，不用配置库中的旧 Key 补足活动认证。

仍需处理的边界：

- 设置打开前，外部 API 配置与已保存条目的归属识别仍不完整，导入草稿可能沿用原条目的名称和 ID。
- 手动移除部分官方令牌或改动认证格式，仍可能被当作退出登录并清除原选中账号的凭据快照。
- 四文件检查与实际写入之间没有跨进程锁，检查后发生的并发写入仍有竞争窗口。
- 历史修复对每个 SQLite 数据库分别提交事务；多个数据库中后一个失败时，前一个已提交数据库不会整体回滚。正在写入的数据库/WAL 文件备份也尚未改用 SQLite 在线备份。

这些是源码检查确认的限制，不属于已通过回归所能保证的范围。完整的行为约定与验证范围见 [应用行为检查](docs/dev-spec/application-behavior.md)。
...
测试覆盖配置库管理、模式切换、OAuth 凭据保存与退出登录、代理校验与进程环境隔离、多文件保存失败恢复，以及历史修复的成功、幂等、备份、数据库失败回滚和保存配置不触发同步。另覆盖 Tab 控件边界与重叠、修复进度可见、保存后保持窗口、未保存修改阻止启动及启动不写配置、打开设置不启动、启动失败保留窗口与重试，并包含大量历史数据、界面响应和商店入口测试。界面测试会短暂打开使用 `example` 配置的窗口，并将截图保存到输出的临时目录；不会实际安装或更新客户端。

### 实现与维护要点

- 官方模式使用内置 `openai` provider；所有自定义 API 使用 `custom`。表单中的“提供者显示名称”只写入 `[model_providers.custom].name`。
