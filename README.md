...
切换 Tab、选择列表条目以及添加、编辑、重命名、删除操作只改变当前草稿。点击当前 Tab 内的“保存配置”写入配置并留在设置页，不启动或停止 ChatGPT；底部“启动”只读取已保存配置，不隐式保存。存在未保存的字段、列表或模式修改时，启动会被阻止并提示先保存。启动器与已运行的 ChatGPT 进程不互斥，启动客户端时启动器随后自动退出。
...
测试覆盖配置库管理、模式切换、OAuth 凭据保存与退出登录、代理校验与进程环境隔离、多文件保存失败恢复，以及历史修复的成功、幂等、备份、数据库失败回滚和保存配置不触发同步。另覆盖 Tab 控件边界与重叠、修复进度可见、保存后保持窗口、未保存修改阻止启动及启动不写配置，并包含大量历史数据、界面响应和商店入口测试。界面测试会短暂打开使用 `example` 配置的窗口，并将截图保存到输出的临时目录；不会实际安装或更新客户端。
