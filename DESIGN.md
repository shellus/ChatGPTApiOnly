...
窗口默认尺寸为 820 × 720，最小尺寸为 600 × 580；页面最大宽度为 960px，居中且至少铺满窗口高度。标题、内容、底部动作按纵向排列，内容区域伸展，底部使用细分隔线。

配置选择器与添加、复制、删除操作位于表单顶部；模型和思考层级在宽窗口中并排。650px 及以下时，内容横向边距降为 18px，字段改单列，配置操作和底部动作允许换行。600px 是桌面窗口下限，不另造手机导航。
