...
窗口首次打开为 700 × 720，之后沿用上次退出时的尺寸、位置与最大化状态；最小尺寸为 600 × 620。页面最大宽度为 960px，居中且至少铺满窗口高度。标题、内容、底部动作按纵向排列，内容区域伸展，底部使用细分隔线。
...
**The Action Placement Rule.** 保存位于当前 Tab 内；启动与关闭固定在底部动作区；修复对话只出现在 Codex 自定义 API 的保存行。不同作用范围的动作不能合并成一个按钮。
...
窗口顶部是客户端选择器：Codex 与 Claude 各自带图标，选中项使用实心强调按钮。两个客户端各自保留自己的模式、配置库与草稿，切换客户端不落盘也不丢弃另一个客户端的编辑。“官方账号”和“自定义 API”使用 Radix Tabs，呈现为分段选择器，选中块在两项之间以 240ms 减速滑动，支持方向键切换与独立焦点环。切换 Tab、选择配置和编辑字段都留在内存草稿中，Tab 不触发落盘。保存行持续显示“有未保存修改”或“配置未修改”。
