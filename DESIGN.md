...
主色使用 Radix `iris`，中性色使用 `slate`。浅色以淡紫白为底，深色以柔和灰紫为底，输入区比背景更亮。页面、控件、文字、主动作与启动动作通过 style.css 中统一的浅深色语义变量定义；蓝紫用于保存和模式选中，珊瑚色突出启动。
...
**The Contrast Rule.** 保存与确认使用 `--action` / `--action-text`，启动使用 `--launch` / `--launch-text`。浅色蓝紫按钮使用白字，深色淡紫按钮和珊瑚按钮使用深色字；正文和操作文字对比度至少 4.5:1。

首次显示跟随系统浅深色偏好；标题区提供带日/月图标与滑动圆点的“浅色 / 深色”切换，仅在当前窗口状态中保留。两种外观复用同一套语义变量。
...
窗口首次打开为 700 × 640，之后沿用上次退出时的尺寸、位置与最大化状态；最小尺寸为 600 × 580。页面最大宽度为 960px，居中且至少铺满窗口高度。标题、内容、底部动作按纵向排列，内容区域伸展，底部使用细分隔线。

**The Measure Rule.** 设置表单在内容区域水平居中，宽度使用 `min(100%, 720px)` 随窗口流式变化，不锁定为固定像素宽度。表单是两列栅格：短值字段各占一列，API 地址与 API Key 跨两列，字段说明放在控件右侧的空列而不是另起一行，官方账号状态与配置名称同行。配置选择器、添加、复制、删除和保存操作与表单同宽并保持居中。
...
“官方账号”和“自定义 API”使用 Radix Tabs，呈现为分段选择器，选中块在两项之间以 240ms 减速滑动，支持方向键切换与独立焦点环。切换 Tab、选择配置和编辑字段都留在内存草稿中，Tab 不触发落盘。保存行持续显示“有未保存修改”或“配置未修改”。

### 按钮与字段

保存使用蓝紫按钮，启动使用珊瑚按钮；添加与复制使用柔和强调按钮；关闭、删除使用灰色柔和按钮；外观切换使用日/月滑块；修复使用灰色描边按钮。安装客户端、检查更新、重新读取使用小号灰色透明按钮，维持辅助层级。
